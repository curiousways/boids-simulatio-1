class Boid {
  ...
  avoidWalls() {
    ...
    if (
      this.distanceFromHorWall() < this.radius * buffer ||
      this.distanceFromVertWall() < this.radius * buffer
    ) {
  ...
  flock() {
    ...
    // Weight Forces
    var alignWeight = 1.2;
    if (mouseSeek) var mouseWeight = 0.2;
  ...
  resolveCollision(boid, otherBoid) {
    ...
    if (xVelocityDiff * xDist + yVelocityDiff * yDist >= 0) {
      ...
      var angle = -Math.atan2(
        otherBoid.position.y - boid.position.y,
        otherBoid.position.x - boid.position.x
      );
